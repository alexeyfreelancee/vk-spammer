...
public class CommentResultListAdapter extends RecyclerView.Adapter<CommentResultListAdapter.ViewHolder> {
    private ArrayList<Result> result_list = new ArrayList<>();
    private Context context;

    public CommentResultListAdapter(ArrayList<Result> result_list, Context context) {
    ...
    public void onBindViewHolder(@NonNull final ViewHolder holder, final int position) {
        final String link = "https://vk.com/wall-"
                +result_list.get(position).getGroupId()+"_" + result_list.get(position).getPostNumber()
                +"?reply="
                + result_list.get(position).getCommentNumber();

        final String link2 = "https://vk.com/public"+result_list.get(position).getGroupId()
                +"?w=wall-"
